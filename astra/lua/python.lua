...
local function resolve_dependencies(prog, path, opts)
  opts = opts or {}
  processed_modules = {}
  local search_dirs = {}
  if opts.path then
  ...
  local src_dir = dirname(path)
  search_dirs[#search_dirs + 1] = src_dir
  add_to_path(src_dir)
...
function python.transpile_file(path, opts)
  ...
  resolve_dependencies(prog, path, opts)
...
function python.run_file(path, opts)
  ...
  python.last_code = lua_code
  local chunk, err = load(lua_code, "=" .. path)
  if not chunk then
    error("Python runtime error: " .. tostring(err))
  end
  return chunk()
