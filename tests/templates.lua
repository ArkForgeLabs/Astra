...
return function(test)
  ...
  describe("Jinja2 Engine", function()
    ...
    describe("Template Management", function()
      it("adds inline template and verifies by name", function()
        local eng = templates.jinja2.new()
        ...
        for _, n in ipairs(names) do
          if n == "test.html" then found = true end
        end
      ...
      it("loads template from file and renders it", function()
        ...
        expect(result).to.match(".*Count is: 3.*")
        expect(result).to.match(".*Home Page.*")
      end)
