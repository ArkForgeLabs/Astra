...
do
  ...
  do
    ...
    local function lex_string_escape(input, i, c)
      ...
      elseif lex_decimals[c] then
        local len = lex_decimals[input:sub(i + 1, i + 1)] and (lex_decimals[input:sub(i + 2, i + 2)] and 2 or 1) or 0
  ...
  do
    ...
    local function parse_trying_list(ps, i, list, parse_item, ret_lookahead)
      ...
      while
        ...
        and (
          not ret_lookahead
          or not (ps.tokens[i + 1].kind == "identifier" and ps.tokens[i + 2] and ps.tokens[i + 2].tk == ":")
        )
      do
        i = i + 1
    ...
    do
      ...
      local function after_valid_prefixexp(ps, prevnode, i)
        return ps.tokens[i - 1].kind == ")"
          or (prevnode.kind == "op" and (prevnode.op.op == "@funcall" or prevnode.op.op == "@index" or prevnode.op.op == "." or prevnode.op.op == ":"))
        ...
        while precedences[2][lookahead] and precedences[2][lookahead] >= min_precedence do
          ...
          while
            ...
            and (
              ...
              or (is_right_assoc[lookahead] and (precedences[2][lookahead] == precedences[2][op.op]))
            )
          do
    ...
    local function get_attached_comments(token)
      ...
      local attached_comments = table.move(token.comments, first_n, #token.comments, 1, {})
    ...
    local function type_needs_local_or_global(ps, i)
      ...
      return failskip(
        ps,
        i,
        ("%s needs to be declared with 'local %s' or 'global %s'"):format(tk, tk, tk),
        skip_type_body
      )
    end
  ...
  local function extra_callback(name, s, ast, xs, visit_node)
  ...
  local function recurse_node(s, root, visit_node, visit_type)
    if not root then
      return
    end
    ...
    recurse = function(ast)
      ...
      if TL_DEBUG then
        ...
        kprint = kind == "op" and "op " .. ast.op.op or kind == "identifier" and "identifier " .. ast.tk or kind
  ...
  function Errors:get_context(ctx, name)
    ...
    local cn = (type(ctx) == "string") and ctx or (ctx.kind ~= nil) and context_name[ec and ec.kind or ctx.kind]
  ...
  function Errors:redeclaration_warning(at, var_name, var_kind, old_var)
    ...
    if old_var and old_var.declared_at then
      self:add_warning(
        "redeclaration",
        at,
        short_error .. " (originally declared at %d:%d)",
        var_name,
        old_var.declared_at.y,
        old_var.declared_at.x
      )
    else
  ...
  local function var_should_be_ignored_for_warnings(name, var)
    ...
    return (not var.declared_at) or var.is_specialized == "narrow" or prefix == "_" or prefix == "@"
  end

  local function user_facing_variable_description(var)
    local t = var.t
    return var.is_func_arg and "argument"
      or t.typename == "function" and "function"
      or t.typename == "typedecl" and "type"
      or "variable"
  end
  ...
  function Errors:add_prefixing(w, src, prefix, dst)
    ...
    for _, err in ipairs(src) do
      ...
      if w and ((err.filename ~= w.f) or not err.y or (w.y > err.y or (w.y == err.y and w.x > err.x))) then
        err.y = w.y
        err.x = w.x
        err.filename = w.f
      end
  ...
  local function display_typevar(typevar, what)
    return TL_DEBUG and (what .. " " .. typevar) or typevar:gsub("@.*", "")
  end
  ...
  local function show_type_base(t, short, seen)
    ...
    elseif t.typename == "function" then
      ...
      for i, v in ipairs(t.args.tuple) do
        table.insert(
          args,
          ((i == #t.args.tuple and t.args.is_va) and "...: " or (i > t.min_arity) and "? " or "") .. show(v)
        )
      end
    ...
    elseif t.typename == "string" then
      if short then
        ...
        return t.typename .. (t.literal and string.format(" %q", t.literal) or "")
      end
  ...
  do
    ...
    function TypeChecker:widen_in_scope(n, var)
      ...
      if (not specialization) or not (specialization == "narrow" or specialization == "narrowed_declaration") then
        return false
      end
        ...
        for fname, ftype in pairs(rec.meta_fields) do
          if ret.fields[fname] then
            if not self:is_a(ftype, ret.fields[fname]) then
              self.errs:add(
                ftype,
                fname .. " does not follow metatable contract: got %s, expected %s",
                ftype,
                ret.fields[fname]
              )
            end
    ...
    do
      ...
      is_lua_table_type = function(t)
        return known_table_types[t.typename] and not (t.fields and t.is_userdata)
      end
    end
    ...
    function TypeChecker:subtype_record(a, b)
      ...
      if a.is_userdata ~= b.is_userdata then
        return false, { Err(a.is_userdata and "userdata is not a record" or "record is not a userdata") }
      end
            ...
            return false,
              {
                Err(
                  "given type %s does not satisfy %s constraint in type variable "
                    .. display_typevar(typevar, "typevar"),
                  other,
                  constraint
                ),
              }
    ...
    local function infer_emptytable_from_unresolved_value(self, w, u, values)
      ...
      if not (values.typename == "emptytable" or values.typename == "unresolved_emptytable_value") then
        local infer_to = is_numeric_type(keys) and a_type(w, "array", { elements = values })
          or a_type(w, "map", { keys = keys, values = values })
    ...
    TypeChecker.eqtype_relations = {
      ...
      ["union"] = {
        ["union"] = function(self, a, b)
          return (self:has_all_types_of(a.types, b.types) and self:has_all_types_of(b.types, a.types))
        end,
      },
      ["nominal"] = {
        ...
        ["function"] = function(self, a, b)
          ...
          if naargs ~= nbargs then
            ...
            return false,
              {
                Err(
                  "different number of input arguments: got " .. naargs - argdelta .. ", expected " .. nbargs - argdelta
                ),
              }
          end
        ...
        ["tupletable"] = function(self, a, b)
          ...
          if alen > #b.types then
            return false,
              {
                Err(
                  "incompatible length, expected maximum length of " .. tostring(#b.types) .. ", got " .. tostring(alen)
                ),
              }
          end
          ...
          for i = 1, (alen > 0) and alen or #b.types do
            if not self:is_a(a.elements, b.types[i]) then
              return false,
                {
                  Err(
                    "tuple entry " .. i .. " of type %s does not match type of array elements, which is %s",
                    b.types[i],
                    a.elements
                  ),
                }
        ...
        ["function"] = function(self, a, b)
          ...
          if (not b.args.is_va) and (self.feat_arity and (#aa > #ba and a.min_arity > b.min_arity)) then
            table.insert(
              errs,
              Err(
                "incompatible number of arguments: got " .. show_arity(a) .. " %s, expected " .. show_arity(b) .. " %s",
                a.args,
                b.args
              )
            )
          ...
          if #ar < #br and not diff_by_va then
            table.insert(
              errs,
              Err("incompatible number of returns: got " .. #ar .. " %s, expected " .. #br .. " %s", a.rets, b.rets)
            )
          else
            local nrets = #br
            if diff_by_va then
          ...
          if not self:find_var_type(a.typearg) then
            if a.constraint then
              ...
              self:add_var(
                nil,
                a.typearg,
                self.feat_lax and a_type(a, "unknown", {})
                  or a_type(a, "unresolvable_typearg", {
                    typearg = a.typearg,
                  })
              )
            end
        ...
        do
          ...
          check_args_rets = function(self, w, wargs, f, args, expected_rets, argdelta, or_args, or_rets)
            ...
            if argdelta == -1 then
              ...
              if
                not (first.typename == "self")
                and not self:arg_check(w, errs, first, args.tuple[1], "contravariant", "self")
              then
                return nil, errs
              end
            end
            ...
            args_ok, args_errs =
              check_func_type_list(self, w, wargs, fargs, args, from, argdelta, "contravariant", "argument")
    ...
    function TypeChecker:add_global(node, varname, valtype, is_assigning)
      ...
      if existing then
        ...
        elseif valtype and not self:same_type(existing.t, valtype) then
          self.errs:add(
            node,
            "cannot redeclare global with a different type: previous type of " .. varname .. " is %s",
            existing.t
          )
        end
        return nil
    ...
    function TypeChecker:type_check_index(anode, bnode, a, b)
      ...
      elseif ra.typename == "emptytable" then
        ...
        errm, erra, errb =
          "inconsistent index type: got %s, expected %s" .. inferred_msg(ra.keys, "type of keys "), b, ra.keys
      ...
      elseif ra.fields then
        if rb.typename == "enum" then
          ...
          if not errm then
            return self:match_all_record_field_names(
              bnode,
              ra,
              field_names,
      ...
      not_facts = function(self, fs)
        ...
        for var, f in pairs(fs) do
          ...
          elseif not self:is_a(f.typ, typ) then
            assert(f.fact == "is")
            self.errs:add_warning(
              "branch",
              f.w,
              f.var .. " (of type %s) can never be a %s",
              show_type(typ),
              show_type(f.typ)
            )
            ret[var] = EqFact({ var = var, typ = a_type(f.w, "invalid", {}), w = f.w, no_infer = f.no_infer })
            ...
            ret[var] = IsFact({
              var = var,
              typ = subtract_types(self, f.w, typ, f.typ),
              w = f.w,
              no_infer = f.no_infer,
            })
          end
    ...
    local special_functions = {
      ["pairs"] = function(self, node, a, b, argdelta)
        ...
        if not (t.typename == "map") then
          if not (self.feat_lax and is_unknown(t)) then
            if t.fields then
              self:match_all_record_field_names(
                node.e2,
                t,
                t.field_order,
      ...
      ["string.find"] = function(self, node, a, b, argdelta)
        ...
        if
          pat.typename == "string"
          and pat.literal
          and ((not plainarg) or (plainarg.kind == "boolean" and plainarg.tk == "false"))
        then
          local st = pat.literal
...
self:expand_type(node, values, elements),
      ...
      if decltype then
        ...
        if infertype then
          if infertype.typename == "unresolvable_typearg" then
            ok = false
            infertype = self.errs:invalid_at(
              node.vars[i],
    ...
    local function recurse_type_declaration(self, n)
      if n.kind == "op" then
        ...
        elseif n.op.op == "@funcall" and n.e1.kind == "variable" and n.e1.tk == "require" then
    ...
    visit_node.cbs = {
          ...
          for i, var in ipairs(node.vars) do
            if var.attribute == "close" then
              if self.gen_target == "5.4" then
                ...
                self.errs:add(
                  var,
                  "<close> attribute is only valid for Lua 5.4 (current target is " .. tostring(self.gen_target) .. ")"
                )
              end
            end
            ...
            if ok and infertype then
              ...
              local rt = self:to_structural(t)
              if
                not (rt.typename == "enum")
                and ((not (t.typename == "nominal")) or (rt.typename == "union"))
          ...
          for i, vartype in ipairs(vartypes) do
            ...
            if err == "missing" then
              if #node.exps == 1 and node_is_funcall(node.exps[1]) then
                local msg = #valtuple.tuple == 1 and "only 1 value is returned by the function"
      ...
      ["forin"] = {
        ...
        before_statements = function(self, node, children)
          ...
          if exp1type.typename == "poly" then
            local _r, f
            _r, f =
              self:type_check_function_call(exp1, exp1type, args, 0, nil, nil, exp1, { node.exps[2], node.exps[3] })
            if f then
              exp1type = f
            else
      ...
      ["fornum"] = {
        before_statements = function(self, node, children)
          ...
          local typename = (
            from_t.typename == "integer"
            and to_t.typename == "integer"
            and (not step_t or step_t.typename == "integer")
          )
              and "integer"
            or "number"
      ...
      ["return"] = {
        ...
        after = function(self, node, children)
          ...
          if n_got > n_expected and not self.feat_lax and not vatype then
            self.errs:add(
              node,
              what .. ": excess return values, expected " .. n_expected .. " %s, got " .. n_got .. " %s",
              expected,
              got
            )
          end
          ...
          if
            ...
            and node.exps[1].discarded_tuple
          then
            self.errs:add_warning(
              "hint",
              node.exps[1].e2,
          ...
          for i = 1, n_got do
            ...
            if e then
              e = resolve_tuple(e)
              local w = (node.exps[i] and node.exps[i].x) and node.exps[i] or node.exps
              assert(w and w.x)
      ...
      ["local_function"] = {
        ...
        after = function(self, node, children)
          ...
          local t = wrap_generic_if_typeargs(
            node.typeargs,
            a_function(node, {
      ...
      ["record_function"] = {
        ...
        before_statements = function(self, node, children)
          ...
          local fn_type = wrap_generic_if_typeargs(
            node.typeargs,
            a_function(node, {
              min_arity = self.feat_arity and node.min_arity or 0,
      ...
      ["function"] = {
        ...
        after = function(self, node, children)
          ...
          return wrap_generic_if_typeargs(
            node.typeargs,
            a_function(node, {
              min_arity = self.feat_arity and node.min_arity or 0,
              args = args,
      ...
      ["op"] = {
        ...
        before_e2 = function(self, node, children)
          ...
          elseif node.op.op == "or" then
            ...
            if
              node.e1.kind == "op"
              and node.e1.op.op == "and"
              and node.e1.e1.kind == "op"
              and node.e1.e1.op.op == "is"
        ...
        after = function(self, node, children)
          ...
          if
            ra.typename == "circular_require"
            or (ra.typename == "typedecl" and ra.def and ra.def.typename == "circular_require")
          then
          ...
          if node.op.op == "or" then
            ...
            elseif is_lua_table_type(ra) and rb.typename == "emptytable" then
              ...
              t = ua
            elseif
              (ra.typename == "enum" and rb.typename == "string" and self:is_a(rb, ra))
              or (ra.typename == "string" and rb.typename == "enum" and self:is_a(ra, rb))
            then
              node.known = nil
          ...
          if node.op.arity == 1 and unop_types[node.op.op] then
            ...
            if not t then
              return self.errs:invalid_at(
                node,
                "cannot use operator '" .. node.op.op:gsub("%%", "%%%%") .. "' on type %s",
                ua
              )
            end
          ...
          if node.op.arity == 2 and binop_types[node.op.op] then
            ...
            elseif bit_operators[node.op.op] and self.gen_target == "5.1" then
              if meta_on_operator then
                self.all_needs_compat["mt"] = true
                convert_node_to_compat_mt_call(
                  node,
                  binop_to_metamethod[node.op.op],
                  meta_on_operator,
                  node.e1,
                  node.e2
                )
            ...
            if not t then
              ...
              return self.errs:invalid_at(
                node,
                "cannot use operator '" .. node.op.op:gsub("%%", "%%%%") .. "' for types %s and %s",
                ua,
                ub
              )
            end
        ...
        after = function(self, node, children)
          ...
          if not t then
            if node.expected and node.tk == "self" then
              ...
              t = self.feat_lax and a_type(node, "unknown", {}) or a_type(node, "any", {})
            end
          end
          if node.tk == "..." then
            t = a_vararg(node, { t })
        ...
        for fname, ftype in fields_of(resolved, list) do
          if fields[fname] then
            if not self:is_a(fields[fname], ftype) then
              ...
              self.errs:add(
                fields[fname],
                what .. " '" .. fname .. "' does not match definition in interface %s",
                named
              )
            end
          else
    ...
    local function patch_visitors(my_visit_node, after_node, my_visit_type, after_type)
    ...
    tl.check = function(ast, filename, opts, env)
      ...
      if opts.run_internal_compiler_checks then
        visit_node, visit_type =
          patch_visitors(visit_node, internal_compiler_check, visit_type, internal_compiler_check)
      end
      if self.collector then
        visit_node, visit_type = patch_visitors(visit_node, store_type_after, visit_type, store_type_after)
      end
      if TL_DEBUG then
        visit_node, visit_type = patch_visitors(visit_node, debug_type_after)
      end
  ...
  tl.init_env = function(lax, gen_compat, gen_target, predefined)
    local opts = {
      defaults = {
        ...
        gen_compat = ((type(gen_compat) == "string") and gen_compat)
          or (gen_compat == false and "off")
          or (gen_compat == true or gen_compat == nil) and "optional",
        gen_target = gen_target or ((_VERSION == "Lua 5.1" or _VERSION == "Lua 5.2") and "5.1") or "5.3",
      },
