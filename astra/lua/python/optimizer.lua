...
local function call_resolution_pass(prog, analysis)
  local func_map = {}
  ...
  local function collect_funcs(body)
    for _, stmt in ipairs(body or {}) do
      ...
      elseif stmt.type == ast.TRY then
        collect_funcs(stmt.body)
        for _, h in ipairs(stmt.handlers or {}) do collect_funcs(h.body) end
        collect_funcs(stmt.finally_body)
      end
    end
  end
  collect_funcs(prog.body)
  ...
  local function resolve_expr(expr)
    ...
    if expr.type == ast.CALL and expr.keywords and #expr.keywords > 0 then
      if expr.func.type == ast.NAME then
        local def = func_map[expr.func.id]
        if def and def.type == ast.FUNCTION_DEF then
          expr._resolved_params = def.args
        end
      end
    end
    if expr.type == ast.CALL then
      resolve_expr(expr.func)
      for _, a in ipairs(expr.args or {}) do resolve_expr(a) end
      for _, kw in ipairs(expr.keywords or {}) do resolve_expr(kw.value) end
    elseif expr.type == ast.BIN_OP then
      resolve_expr(expr.left); resolve_expr(expr.right)
    elseif expr.type == ast.UNARY_OP then resolve_expr(expr.operand)
    elseif expr.type == ast.BOOL_OP then
      for _, v in ipairs(expr.values) do resolve_expr(v) end
    elseif expr.type == ast.COMPARE then
      resolve_expr(expr.left)
      for _, c in ipairs(expr.comparators) do resolve_expr(c) end
    elseif expr.type == ast.SUBSCRIPT then
      resolve_expr(expr.value); resolve_expr(expr.index)
    elseif expr.type == ast.ATTRIBUTE then resolve_expr(expr.value)
    elseif expr.type == ast.LIST or expr.type == ast.SET or expr.type == ast.TUPLE then
      for _, e in ipairs(expr.elements or {}) do resolve_expr(e) end
    elseif expr.type == ast.DICT then
      for _, k in ipairs(expr.keys or {}) do resolve_expr(k) end
      for _, v in ipairs(expr.values or {}) do resolve_expr(v) end
    elseif expr.type == ast.LAMBDA then resolve_expr(expr.body)
    elseif expr.type == ast.WALRUS then resolve_expr(expr.target); resolve_expr(expr.value)
    elseif expr.type == ast.IF_EXPR then
      resolve_expr(expr.test); resolve_expr(expr.body); resolve_expr(expr.or_else)
    elseif expr.type == ast.LIST_COMP or expr.type == ast.SET_COMP then
      resolve_expr(expr.element)
      for _, g in ipairs(expr.generators or {}) do
        resolve_expr(g.iterator)
        for _, if_expr in ipairs(g.ifs or {}) do resolve_expr(if_expr) end
      end
    elseif expr.type == ast.DICT_COMP then
      resolve_expr(expr.key); resolve_expr(expr.value)
      for _, g in ipairs(expr.generators or {}) do
        resolve_expr(g.iterator)
        for _, if_expr in ipairs(g.ifs or {}) do resolve_expr(if_expr) end
      end
    elseif expr.type == ast.STARRED then resolve_expr(expr.value)
    end
  end
  ...
  local function resolve_body(body)
    for _, stmt in ipairs(body or {}) do
      ...
      elseif stmt.type == ast.IF then
        resolve_body(stmt.body)
        for _, elif in ipairs(stmt.elifs or {}) do resolve_body(elif.body) end
      ...
      elseif stmt.type == ast.TRY then
        resolve_body(stmt.body)
        for _, h in ipairs(stmt.handlers or {}) do resolve_body(h.body) end
        resolve_body(stmt.finally_body)
      elseif stmt.type == ast.EXPR_STMT then
        resolve_expr(stmt.expr)
      elseif stmt.type == ast.ASSIGN then
        for _, t in ipairs(stmt.targets or {}) do resolve_expr(t) end
        resolve_expr(stmt.value)
      elseif stmt.type == ast.AUG_ASSIGN then
        resolve_expr(stmt.target); resolve_expr(stmt.value)
      elseif stmt.type == ast.RETURN then
        if stmt.value then resolve_expr(stmt.value) end
      end
    end
  end

  resolve_body(prog.body)
end
...
local function if_false_pass(prog)
    ...
    while i <=# body do
      ...
      if stmt.type == ast.IF then
        ...
        elseif is_constant(stmt.test, true) then
          local inline = stmt.body or {}
          if stmt.or_else then
            for _, s in ipairs(stmt.or_else) do inline[#inline + 1] = s end
          end
          body[i] = inline
        else
          i = i + 1
  ...
  local function visit_node(stmt)
    ...
    if stmt.type == ast.IF then
      ...
      for _, elif in ipairs(stmt.elifs or {}) do visit_body(elif.body) end
    ...
    elseif stmt.type == ast.TRY then
      visit_body(stmt.body)
      for _, h in ipairs(stmt.handlers or {}) do visit_body(h.body) end
  ...
  local function walk(body)
    if not body then return end
    for _, stmt in ipairs(body) do visit_node(stmt) end
    visit_body(body)
  end
...
local function while_false_pass(prog)
  local function walk(body)
    if not body then return end
    local i = 1
    while i <=# body do
  ...
  local function visit(stmt)
    ...
    if stmt.type == ast.IF then
      walk(stmt.body)
      for _, elif in ipairs(stmt.elifs or {}) do walk(elif.body) end
      walk(stmt.or_else)
    elseif stmt.type == ast.FUNCTION_DEF or stmt.type == ast.CLASS_DEF then
      walk(stmt.body)
    elseif stmt.type == ast.WHILE or stmt.type == ast.FOR then
    ...
    elseif stmt.type == ast.TRY then
      walk(stmt.body)
      for _, h in ipairs(stmt.handlers or {}) do walk(h.body) end
  ...
  local function recurse(body)
    if not body then return end
    for _, stmt in ipairs(body) do visit(stmt) end
    walk(body)
  end
...
function optimizer.analyze(prog, opts)
  ...
  if optimize and opts.call_resolution ~= false then
    call_resolution_pass(prog, analysis)
  end
  if optimize and opts.if_false_prune ~= false then
    if_false_pass(prog)
  end
  if optimize and opts.while_false_prune ~= false then
    while_false_pass(prog)
  end
