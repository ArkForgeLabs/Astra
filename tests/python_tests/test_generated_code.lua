...
return function(test)
  test.it("generates inline subscript for list access", function()
    ...
    test.expect(lua).to.match("__py_getitem%(items, 1%)")
  end)
  ...
  test.it("preserves multi-line comments as block", function()
    ...
    test.expect(lua).to.match("%-%-  line 1")
    test.expect(lua).to.match("%-%-  line 2")
    test.expect(lua).to.match("x = 1")
  end)

  test.it("preserves docstring as commented lines with blank lines", function()
    local lua = python.transpile('"""\nTitle\n\nBody\n"""\nx = 1')
    test.expect(lua).to.match("%-%- Title")
    test.expect(lua).to.match("%-%- Body")
    test.expect(lua).to.match("x = 1")
  end)

  test.it("preserves comment spacing exactly", function()
    local lua = python.transpile("#   indented comment\nx = 1")
    test.expect(lua).to.match("%-%-    indented comment")
  end)

  test.it("preserves comments after return statement", function()
    local lua = python.transpile("def f():\n    return 1\n    # after return\n    pass")
    test.expect(lua).to.match("return 1")
    test.expect(lua).to.match("%-%-  after return")
  end)
  ...
  test.it("generates from import as inline value", function()
    local lua = python.transpile("from math import sqrt")
    test.expect(lua).to.match("sqrt = %(require%(\"python%.stdlib%.math\"%)%).sqrt")
  end)

  test.it("generates from import with alias inline", function()
    local lua = python.transpile("from math import sqrt as sq")
    test.expect(lua).to.match("sq = %(require%(\"python%.stdlib%.math\"%)%).sqrt")
  end)

  test.it("generates multi-name from import", function()
    local lua = python.transpile("from sys import argv, exit")
    test.expect(lua).to.match("argv = %(require%(\"python%.stdlib%.sys\"%)%).argv")
    test.expect(lua).to.match("exit = %(require%(\"python%.stdlib%.sys\"%)%).exit")
  end)

  test.it("generates from import *", function()
    local lua = python.transpile("from math import *")
    test.expect(lua).to.match("for k, v in pairs%(require%('python%.stdlib%.math'%)%) do _ENV%[k%] = v end")
  end)
