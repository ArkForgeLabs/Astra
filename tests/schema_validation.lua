...
return function(test)
  ...
  describe("build()", function()
    ...
    it("propagates defaults through optional", function()
      ...
      local u, err = User({})
      expect(u).to.be.truthy()
      expect(u.name).to.equal("anonymous")
      expect(u.email).to.equal("none@example.com")
    end)
    ...
    it(":validate() rejects invalid data on built struct", function()
      local Point = t.build(t.struct({ x = t.number(), y = t.number() }))
      local ok, _ = t.validate(Point, { x = "bad" })
